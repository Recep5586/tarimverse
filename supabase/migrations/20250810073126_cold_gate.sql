/*
  ...
  1. New Tables
    - `crops`
      - `id` (uuid, primary key)
      - `name` (text, required)
      - `scientific_name` (text, optional)
      - `category` (text, required)
      - `planting_season` (text array, required)
      - `growing_days` (integer, required)
      - `soil_type` (text array, optional)
      - `water_needs` (text, required)
      - `sun_requirements` (text, required)
      - `companion_plants` (text array, optional)
      - `avoid_plants` (text array, optional)
      - `planting_depth_cm` (integer, optional)
      - `spacing_cm` (integer, optional)
    ...
    - `planting_calendar`
      ...
      - `planting_month` (integer, required)
      - `harvest_month` (integer, required)
  ...
  4. Sample Data
    - Insert common crops with their planting information
    - Add planting calendar entries for different regions
*/
...
-- Create policies for crops (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'crops' AND policyname = 'Anyone can read crops'
  ) THEN
...
-- Create policies for planting_calendar (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'planting_calendar' AND policyname = 'Anyone can read planting calendar'
  ) THEN
...
-- Insert some sample crop data (only if table is empty)
INSERT INTO crops (name, scientific_name, category, planting_season, growing_days, soil_type, water_needs, sun_requirements, planting_depth_cm, spacing_cm, temperature_min, temperature_max, description, tips, image_url) 
SELECT * FROM (VALUES
...
) AS v(name, scientific_name, category, planting_season, growing_days, soil_type, water_needs, sun_requirements, planting_depth_cm, spacing_cm, temperature_min, temperature_max, description, tips, image_url)
WHERE NOT EXISTS (SELECT 1 FROM crops WHERE crops.name = v.name);
